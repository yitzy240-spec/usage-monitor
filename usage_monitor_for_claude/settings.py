    ...
    for key, value in data.items():
        if key in _NUMERIC_BOUNDS:
            ...
            if value == 'auto':
                ...
                isinstance(v, bool) or not isinstance(v, (int, float)) or v < 1 for v in value
            ):
                errors.append(f'  {key}: expected "auto" or a [width, height] pair')
                drop.append(key)
            else:
...
# HUD size: 'auto' (fixed width, content-driven height) or a manual
# [width, height] in logical pixels (written back on user edge-resize).
HUD_SIZE: str | list = _S.get('hud_size', 'auto')
